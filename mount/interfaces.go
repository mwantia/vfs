...
type MountPoint interface {
	...
	Unmount(traversal context.TraversalContext, force bool) error
	...
	OpenFile(traversal context.TraversalContext, flags data.AccessMode) (MountStreamer, error)
	...
	CloseFile(traversal context.TraversalContext, force bool) error

	// Read reads size bytes from the file at path starting at offset.
	// Returns the data read or an error if the operation fails.
	ReadFile(traversal context.TraversalContext, offset, size int64) ([]byte, error)
	...
	WriteFile(traversal context.TraversalContext, offset int64, buffer []byte) (int, error)

	// Stat returns file information for the given path.
	// Returns an error if the path doesn't exist.
	StatMetadata(traversal context.TraversalContext) (*data.Metadata, error)

	// Lookup checks if a file or directory exists at the given path.
	// Returns true if the path exists, false otherwise.
	LookupMetadata(traversal context.TraversalContext) (bool, error)
	...
	ReadDirectory(traversal context.TraversalContext) ([]*data.Metadata, error)

	// CreateDirectory creates a new directory at the specified path.
	// Returns an error if the directory already exists or cannot be created.
	CreateDirectory(traversal context.TraversalContext) error
	...
	RemoveDirectory(traversal context.TraversalContext, force bool) error

	// UnlinkFile removes a file at the specified path.
	// Returns an error if the path is a directory or doesn't exist.
	UnlinkFile(traversal context.TraversalContext) error
	...
	Rename(traversal context.TraversalContext, path string) error
}
